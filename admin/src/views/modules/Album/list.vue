<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      ...
      <el-form-item :label="contents.inputTitle == 1 ? 'Album' : ''">
      <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search" v-model="searchForm.albumtitle" placeholder="Album" clearable></el-input>
      <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search" v-model="searchForm.albumtitle" placeholder="Album" clearable></el-input>
      <el-input v-if="contents.inputIcon == 0" v-model="searchForm.albumtitle" placeholder="Album" clearable></el-input>
      </el-form-item>
      ...
      <el-input v-if="contents.inputIcon == 0" v-model="searchForm.clubtype" placeholder="Type" clearable></el-input>
        ...
        <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
          <el-form-item>
            <el-button
              v-if="isAuth('Album','Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="success"
              icon="el-icon-plus"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'Add':'' }}</el-button>
            <el-button
              v-if="isAuth('Album','Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="success"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'Add':'' }}<i class="el-icon-plus el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('Album','Add') && contents.btnAdAllIcon == 0"
              type="success"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'Add':'' }}</el-button>
            <el-button
              v-if="isAuth('Album','Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
            ...
            <el-button
              v-if="isAuth('Album','Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"
              type="danger"
              @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1?'Delete':'' }}<i class="el-icon-delete el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('Album','Delete') && contents.btnAdAllIcon == 0 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"
              type="danger"
              @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1?'Delete':'' }}</el-button>
      ...
      <div class="table-content">
        ...
        <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="coverphoto"
                    header-align="center"
                    width="200"
                    label="Album Cover">
        <template slot-scope="scope">
         <div v-if="scope.row.coverphoto">
             <img :src="scope.row.coverphoto.split(',')[0]" width="100" height="100">
         </div>
        ...
        <el-table-column width="300" :align="contents.tableAlign"
                ...
                label="OK">
         <template slot-scope="scope">
                <el-button v-if="isAuth('Album','View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'Detail':'' }}</el-button>
                <el-button v-if="isAuth('Album','View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'Detail':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('Album','View') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'Detail':'' }}</el-button>
                <el-button v-if="isAuth('Album','Update') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Update':'' }}</el-button>
                <el-button v-if="isAuth('Album','Update') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Update':'' }}<i class="el-icon-edit el-icon--right" /></el-button>
                <el-button v-if="isAuth('Album','Update') && contents.tableBtnIcon == 0" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Update':'' }}</el-button>




                <el-button v-if="isAuth('Album','Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Delete':'' }}</el-button>
                <el-button v-if="isAuth('Album','Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="danger" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Delete':'' }}<i class="el-icon-delete el-icon--right" /></el-button>
                <el-button v-if="isAuth('Album','Delete') && contents.tableBtnIcon == 0" type="danger" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Delete':'' }}</el-button>
                </template>
            </el-table-column>
...
init () {                                                                                                                                                                                                                                                },
    ...
    getDataList() {
      ...
      if(this.searchForm.albumtitle!='' && this.searchForm.albumtitle!=undefined){
            params['albumtitle'] = '%' + this.searchForm.albumtitle + '%'
          }
