  ...
  <div v-if="showFlag">
    ...
    <el-button v-if="contents.searchBtnIcon == 0" type="success" @click="search()">{{ contents.searchBtnFont == 1?'Search':'' }}</el-button>
        ...
        <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
          <el-form-item>
            <el-button
              v-if="isAuth('ClubInfo','Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="success"
              icon="el-icon-plus"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'Add':'' }}</el-button>
            <el-button
              v-if="isAuth('ClubInfo','Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="success"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'Add':'' }}<i class="el-icon-plus el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('ClubInfo','Add') && contents.btnAdAllIcon == 0"
              type="success"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'Add':'' }}</el-button>
            <el-button
              v-if="isAuth('ClubInfo','Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
            ...
            <el-button
              v-if="isAuth('ClubInfo','Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"
              type="danger"
              @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1?'Delete':'' }}<i class="el-icon-delete el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('ClubInfo','Delete') && contents.btnAdAllIcon == 0 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"
              type="danger"
              @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1?'Delete':'' }}</el-button>
        ...
        <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
		    ...
		    label="Address">
		     <template slot-scope="scope">
                ...
                label="OK">
                <template slot-scope="scope">
        <el-button v-if="isAuth('ClubInfo','View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'Detail':'' }}</el-button>
        <el-button v-if="isAuth('ClubInfo','View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'Detail':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
        <el-button v-if="isAuth('ClubInfo','View') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'Detail':'' }}</el-button>
        <el-button v-if="isAuth('ClubInfo','Apply') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="ClubApplicationCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'Apply':'' }}</el-button>
        <el-button v-if="isAuth('ClubInfo','Apply') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="ClubApplicationCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'Apply':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
        <el-button v-if="isAuth('ClubInfo','Apply') && contents.tableBtnIcon == 0" type="success" size="mini" @click="ClubApplicationCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'Apply':'' }}</el-button>
        <el-button v-if="isAuth('ClubInfo','Update') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Update':'' }}</el-button>
        <el-button v-if="isAuth('ClubInfo','Update') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Update':'' }}<i class="el-icon-edit el-icon--right" /></el-button>
        <el-button v-if="isAuth('ClubInfo','Update') && contents.tableBtnIcon == 0" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Update':'' }}</el-button>




        <el-button v-if="isAuth('ClubInfo','Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Delete':'' }}</el-button>
        <el-button v-if="isAuth('ClubInfo','Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="danger" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Delete':'' }}<i class="el-icon-delete el-icon--right" /></el-button>
        <el-button v-if="isAuth('ClubInfo','Delete') && contents.tableBtnIcon == 0" type="danger" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'Delete':'' }}</el-button>
     </template>
        </el-table-column>
    ...
    <ClubApplication-cross-add-or-update v-if="ClubApplicationCrossAddOrUpdateFlag" :parent="this" ref="ClubApplicationCrossaddOrUpdate"></ClubApplication-cross-add-or-update>
...
import ClubApplicationCrossAddOrUpdate from "../ClubApplication/add-or-update";
export default {
  data() {
    return {                                                                                                                                    searchForm: {
        key: ""
      ...
      addOrUpdateFlag:false,
                  ClubApplicationCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
            ClubApplicationCrossAddOrUpdate,
          },
  methods: {
    contentStyleChange() {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            ClubApplicationCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.ClubApplicationCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','ClubInfo');
      this.$nextTick(() => {
        this.$refs.ClubApplicationCrossaddOrUpdate.init(row.id,type);
      });
    },
 init () {                                                                                                                                                                                                                                                           },
    search() {
