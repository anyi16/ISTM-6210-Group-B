<?php
session_start();
class ClubApplicationController extends CommonController {
    ...
    public function page(){
        ...
        $sql = "select * from `ClubApplication` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
                $lists = "select * from `ClubApplication` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `ClubApplication` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE ClubApplication SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    ...
    public function delete(){
        ...
        $str = implode(',',$arr[0]);
        $sql = "delete from ClubApplication WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `ClubApplication` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
    ...
    public function remind($columnName,$type){
        ...
        if ($type == 1){
            $sql = "select * from `ClubApplication` where ".$columnName."<='".$remindEnd."' and ".$columnName.">='".$remindStart."'";
        }else{
            $sql = "select * from `ClubApplication` where ".$columnName."<='".date("Y-m-d",strtotime("+".$remindStart." day"))."' and ".$columnName.">='".date("Y-m-d",strtotime("+".$remindStart." day"))."'";
        }
        $result = table_sql($sql);
