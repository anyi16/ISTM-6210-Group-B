<?php
session_start();
class EventApplicationController extends CommonController {
    ...
    public function page(){
        ...
        if ($base['isAdmin']!=1){
            ...
            $columData = $base['columData'];
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == 'OrdinaryUser'){
                    $where .= " and `ordinaryusername` = '".$_SESSION[$md5]."'";
                }
            }
        }
                                                                        		        
        $sql = "select * from `EventApplication` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
                $lists = "select * from `EventApplication` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `EventApplication` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE EventApplication SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    ...
    public function delete(){
        ...
        $str = implode(',',$arr[0]);
        $sql = "delete from EventApplication WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `EventApplication` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
    ...
    public function remind($columnName,$type){
        ...
        if ($type == 1){
            $sql = "select * from `EventApplication` where ".$columnName."<='".$remindEnd."' and ".$columnName.">='".$remindStart."'";
        }else{
            $sql = "select * from `EventApplication` where ".$columnName."<='".date("Y-m-d",strtotime("+".$remindStart." day"))."' and ".$columnName.">='".date("Y-m-d",strtotime("+".$remindStart." day"))."'";
        }
        $result = table_sql($sql);
