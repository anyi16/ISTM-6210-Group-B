<?php
session_start();
class OrdinaryUserController extends CommonController {
    ...
    public function login(){
        ...
        $password = isset($_REQUEST['password'])?$_REQUEST['password']:"";
                        $sql = "select * from `OrdinaryUser` where `ordinaryusername` = '".$username."' and `password` = '".$password."'";                                                                                         $result = table_sql($sql);
        if ($result->num_rows > 0) {
            ...
            exit(json_encode(['code'=>500,'msg'=>"Wrong username or password"]));
        }
    }
    ...
    public function register(){
        ...
        $postData = json_decode($tmpData,true);
                        $colum = "ordinaryusername";
        $trues = "select * from `OrdinaryUser` where `ordinaryusername` = '".$postData[$colum]."'";                                                                                                		        $result = table_sql($trues);
        if($result->num_rows<1){
			...
			$key = implode(',',$keyArr);
			$v = implode(',',$valArr);
                        $sql = "INSERT INTO `OrdinaryUser` (`id`,".$key.") VALUES (".time().",".$v.")";
                        $result = table_sql($sql);
            if (!$result) exit(json_encode(['code'=>500,'msg'=>'Registration failed']));
            exit(json_encode(['code'=>0]));
        }
        exit(json_encode(['code'=>500,'msg'=>"The user name already exist"]));
    }
    ...
    public function resetPass(){
        $username = input('post.username');
                        $counts = "select * from `OrdinaryUser` where ordinaryusername = '".$username."'";
        $cotte = table_sql($counts);
        if($cotte->num_rows<1){
            exit(json_encode(['code'=>500,'msg'=>"Wrong username."]));
        }
        $sql = "update OrdinaryUser set password = '123456' where ordinaryusername = '".$username."'";                                                                                                       		        $result = table_sql($sql);
    ...
    public function page(){
        ...
        if ($base['isAdmin']!=1){
            ...
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == '${column.authTable}'){
                    $where .= " and `ordinaryusername` = '".$_SESSION[$md5]."'";
                }
            }
        }
                                                                                                        		        
        $sql = "select * from `OrdinaryUser` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
                $lists = "select * from `OrdinaryUser` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function lists(){
        ...
        if ($base['isAdmin']!=1){
            ...
            if (isset($base['success'][$colum]) && in_array($colum, $columData)){
                $where .= " and `ordinaryusername` = '".$base['success'][$colum]."'";
            }
        }                                                                                                       		    $sql = "select * from `OrdinaryUser`".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
    ...
    public function save(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `OrdinaryUser` (`id`,".$k.") VALUES (".time().",".$v.")";
                $result = table_sql($sql);
		        exit(json_encode(['code'=>0]));
    }
    ...
    public function add(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                $sql = "INSERT INTO `OrdinaryUser` (".$k.") VALUES (".$v.")";
        $result = table_sql($sql);
		        exit(json_encode(['code'=>0]));
    }
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE OrdinaryUser SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    ...
    public function delete(){
        ...
        $str = implode(',',$arr[0]);
        $sql = "delete from OrdinaryUser WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `OrdinaryUser` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
    ...
    public function remind($columnName,$type){
        ...
        if ($type == 1){
            $sql = "select * from `OrdinaryUser` where ".$columnName."<='".$remindEnd."' and ".$columnName.">='".$remindStart."'";
        }else{
            $sql = "select * from `OrdinaryUser` where ".$columnName."<='".date("Y-m-d",strtotime("+".$remindStart." day"))."' and ".$columnName.">='".date("Y-m-d",strtotime("+".$remindStart." day"))."'";
        }
        $result = table_sql($sql);
